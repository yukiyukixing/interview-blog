...
>在 React 中，"副作用"（side effects）指的是那些超出渲染流程的操作。这些操作通常涉及与外部系统的交互，例如发起网络请求、直接操作 DOM、设置订阅以及定时器等。副作用可能会影响其他组件或与外部世界交互，因此它们不能在纯渲染过程中进行。

## 5.useMemo，React.memo，useCallback差别 ⭐

- 1.useMemo：用于缓存计算的结果，从而避免在每次渲染时重复执行昂贵的计算。

- 2.useCallback：用于记住函数实例本身的，useCallback主要用于避免父组件渲染导致子组件不必要的渲染

- 3.React.memo：React.memo 是一个高阶组件。它用于对组件进行性能优化，通过记忆组件输出来减少不必要的渲染。
